...
internal sealed class CategoryRepository : ICategoryRepository
{
    ...
    public async Task<AddCategoryResult> AddOrRestoreAsync(long userId, string name, string requestId, CancellationToken cancellationToken)
    {
        ...
        // Нормализация имени
        string collapsed = CollapseSpaces(name ?? string.Empty);
        if (string.IsNullOrWhiteSpace(collapsed))
            return new AddCategoryResult(false, false, null);

        string capitalized = Capitalize(collapsed);
        string normalizedLower = capitalized.ToLowerInvariant();
        ...
        bool activeExists = await _dbContext.Categories
            ...
            .AnyAsync(c => c.UserId == userId
                           && !c.IsDeleted
                           && !c.IsSystem
                           && c.Name.ToLower() == normalizedLower, cancellationToken);
        if (activeExists)
            return new AddCategoryResult(false, false, null);
        ...
        CategoryEntity? deleted = await _dbContext.Categories
            .Where(c => c.UserId == userId
                        && c.IsDeleted
                        && !c.IsSystem
                        && c.Name.ToLower() == normalizedLower)
    ...
    private static string Capitalize(string input)
    {
        ...
        // кофе -> Кофе todo: может и отказаться от этого? Писать все в lower case а при выводе нормализовывать?
